            ...
            if (milestoneId === "new" && newMilestoneTitle) {
                ...
                const res = await fetch("/api/milestones", {
                    ...
                    body: JSON.stringify({
                        ...
                        targetDate: new Date(new Date().setFullYear(new Date().getFullYear() + 1)).toISOString().split("T")[0], // Default 1 year out
                        linkedCourseIds: [],
                    }),
                });
                ...
                >
                    <div className="flex flex-col gap-3">
                        <OptionCard 
                            ...
                            onClick={() => { setMilestoneId("none"); nextScreen(); }} 
                        />
                        ...
                        {milestones.map(m => (
                            <OptionCard 
                                ...
                                label={m.title} 
                                onClick={() => { setMilestoneId(m.id); nextScreen(); }} 
                            />
                        ))}
                        <OptionCard 
                        ...
                        {milestoneId === "new" && (
                            <div className="mt-4 p-4 border border-border rounded-xl bg-muted/30 animate-in fade-in zoom-in-95">
                                ...
                                />
                                <Button className="w-full" disabled={!newMilestoneTitle.trim()} onClick={nextScreen}>Save Goal & Continue</Button>
                            </div>
                        )}
                    </div>
                </ScreenWrapper>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        {["YouTube", "Coursera", "Udemy", "Duolingo", "Book / Notes", "Notion", "College", "Other"].map(opt => (
                            <OptionCard 
                                ...
                                selected={source === opt} 
                                label={opt} 
                                onClick={() => { setSource(opt); nextScreen(); }} 
                            />
                        ))}
                    </div>
                </ScreenWrapper>
                    ...
                    <div className="flex flex-col gap-3">
                        {[
                            ...
                            <OptionCard 
                                ...
                                label={opt.label} 
                                onClick={() => { setConsistencyGoal(opt.id); nextScreen(); }} 
                            />
                        ))}
                    </div>
                </ScreenWrapper>
                    ...
                    <div className="flex flex-col gap-3">
                        {consistencyGoal === "every_day" && ["15 minutes", "30 minutes", "1 hour", "Other"].map(opt => (
                            <OptionCard 
                                ...
                                label={opt} 
                                onClick={() => { setConsistencyDetails(opt); nextScreen(); }} 
                            />
                        ))}
                        
                        {consistencyGoal === "few_times_week" && ["2 days", "3 days", "4 days", "5 days"].map(opt => (
                            <OptionCard 
                                key={opt}
                                selected={consistencyDetails === opt} 
                                label={opt} 
                        ...
                        {consistencyGoal === "decide_later" && (
                            <div className="flex justify-center p-8">
                                <Button onClick={nextScreen} size="lg">Continue</Button>
                            </div>
                        )}
                    </div>
                </ScreenWrapper>
                    ...
                    <div className="flex flex-col gap-3">
                        {[
                            ...
                            <OptionCard 
                                ...
                                label={opt.label} 
                                onClick={() => { setProgressStyle(opt.id); nextScreen(); }} 
                            />
                        ))}
                    </div>
                </ScreenWrapper>
                    ...
                    <div className="flex flex-col gap-3">
                        {[
                            ...
                            <OptionCard 
                                ...
                                label={opt.label} 
                                onClick={() => { setMissedDaysTone(opt.id); nextScreen(); }} 
                            />
                        ))}
                    </div>
                </ScreenWrapper>
